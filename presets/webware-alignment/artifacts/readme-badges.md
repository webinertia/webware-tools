...
(e.g. `0.1.x`). The codecov badge URL carries no `?branch=` parameter so it
always points at the default branch. The Stryker badge URL embeds the branch
segment — update it in both the badge URL and the dashboard link whenever the
default branch changes.

There is deliberately no CI badge. The pipeline is the organization's required
workflow, owned by `webinertia/.github`; a required workflow has no file in the
consumer repository, so every candidate URL returns 404
(`.../<repo>/actions/workflows/continuous-integration.yml/badge.svg` and
`.../org-required-ci.yml/badge.svg` both do). Scaffolding one produces a broken
image.
